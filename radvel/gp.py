...
class Matern32Kernel(Kernel):
    """
    Class that computes and stores a Matern kernel with :math:`\\nu = 3/2`.
    An arbitrary element, :math:`C_{ij}`, of the matrix is:

    .. math::

        C_{ij} = \\eta_1^2 \\left( 1 + \\frac{\\sqrt{3} |t_i - t_j|}{\\eta_2} \\right)\\,
                 \\exp \\left( \\frac{\\sqrt{3} |t_i - t_j|}{\\eta_2} \\right)
    ...
    def __init__(self, hparams):
        ...
        assert len(hparams) == 2, (
            "Matern32Kernel requires exactly 2 hyperparameters with names"
            + "'gp_length*' and 'gp_amp*'."
        )
        ...
        try:
            ...
            raise KeyError(
                "Matern32Kernel requires hyperparameters 'gp_length*'"
                + " and 'gp_amp*'."
            )
        except AttributeError:
            raise AttributeError(
                "Matern32Kernel requires dictionary of"
                + " radvel.Parameter objects as input."
            )
    ...
    def __repr__(self):
        length = self.hparams["gp_length"].value
        amp = self.hparams["gp_amp"].value
        return "Matern32 Kernel with length: {}, amp: {}".format(length, amp)
    ...
    def compute_covmatrix(self, errors):
        ...
        dterm = np.sqrt(3) * self.dist / length
        K = amp ** 2 * (1 + dterm) * scipy.exp(-dterm)
...
class Matern52Kernel(Kernel):
    ...
    .. math::
        ...
        C_{ij} = \\eta_1^2 \\left( 1 + \\frac{\\sqrt{5} |t_i - t_j|}{\\eta_2} + \\frac{5 |t_i - t_j|^2}{3 \\eta_2}\\right)\\,
                 \\exp \\left( \\frac{\\sqrt{5} |t_i - t_j|}{\\eta_2} \\right)
    ...
    def __init__(self, hparams):
        ...
        assert len(hparams) == 2, (
            "Matern52Kernel requires exactly 2 hyperparameters with names"
            + "'gp_length*' and 'gp_amp*'."
        )
        ...
        try:
            ...
            raise KeyError(
                "Matern52Kernel requires hyperparameters 'gp_length*'"
                + " and 'gp_amp*'."
            )
        except AttributeError:
            raise AttributeError(
                "Matern52Kernel requires dictionary of"
                + " radvel.Parameter objects as input."
            )
    ...
    def __repr__(self):
        length = self.hparams["gp_length"].value
        amp = self.hparams["gp_amp"].value
        return "Matern52 Kernel with length: {}, amp: {}".format(length, amp)

    def compute_distances(self, x1, x2):
        X1 = np.array([x1]).T
        X2 = np.array([x2]).T
        self.dist = scipy.spatial.distance.cdist(X1, X2, "euclidean")
        self.dist2 = scipy.spatial.distance.cdist(X1, X2, "sqeuclidean")
